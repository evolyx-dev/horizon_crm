...
test.describe("Permission — RBAC negative cases", () => {
  ...
  test("Guest cannot write to inquiry", async ({ browser }) => {
    const adminCtx = await browser.newContext();
    const adminPage = await adminCtx.newPage();
    await login(adminPage, USERS.admin.email, USERS.admin.password);
    ...
    const inq = await createDoc(adminPage, "Travel Inquiry", {
      customer: custBody.data[0].name,
      customer_name: "Test",
    ...
    const resp = await guestPage.request.put(
      `/api/resource/Travel Inquiry/${inq.data.name}`,
      {
        data: { status: "Won" },
      }
    );
    expect([400, 401, 403, 404, 409]).toContain(resp.status());
